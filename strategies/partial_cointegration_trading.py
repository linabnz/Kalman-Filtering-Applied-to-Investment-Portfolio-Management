        ...
        for t in range(self.rolling_window, len(y)):
            ...
            if open_trade is None:
                ...
                if z > self.entry_threshold:
                    ...
                    trades_log.append(
                        {"date": date, "position": 1, "z_score": z, "mr_component": mr_t, "rw_component": rw_t, "pnl": 0}
                    )
                ...
                # Track the trade
                trades_log.append(
                    {"date": date, "position": -1 if open_trade["side"] == "long_y_short_x" else 1, 
                     "z_score": z, "mr_component": mr_t, "rw_component": rw_t, "pnl": pnl}
                )
                    ...
                    self.trades.append(
                        {
                            ...
                            "exit_price_x": x_t,
                            "pnl": pnl,
                            "exit_reason": "zero_crossing" if abs(z) < 0.1 else "stop_loss" if pnl < -self.stop_loss else "profit_target"
                        }
                    )
                    trades_log.append(
                        {"date": date, "position": 0, "z_score": z, "mr_component": mr_t, "rw_component": rw_t, "pnl": pnl}
                    )
                    open_trade = None
                    frozen_model = None

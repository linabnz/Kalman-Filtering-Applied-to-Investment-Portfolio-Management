...
def download_jse_data(start_date='2015-01-01', end_date=None, min_history_years=5):
    ...
    print(f"Téléchargement de données pour {len(all_tickers)} actions JSE de {start_date} à {end_date}...")
            ...
            if len(batch_tickers) == 1:
                ...
                for ticker in batch_tickers:
                    if ticker in batch_data.columns:
                        ...
                        print(f"Attention: {ticker} non trouvé dans les données téléchargées")
    ...
    valid_tickers = missing_values[missing_values < (len(all_data) - min_history_days)].index.tolist()
    
    if len(valid_tickers) < len(all_tickers):
        print(f"Suppression de {len(all_tickers) - len(valid_tickers)} actions avec historique insuffisant")
    ...
    pd.DataFrame(list(ticker_to_sector.items()), columns=['Ticker', 'Sector']).to_csv('data/jse_sectors.csv', index=False)
    ...
    stats = pd.DataFrame({
        ...
        'Secteur': [ticker_to_sector.get(ticker, 'Inconnu') for ticker in all_data.columns]
    })
